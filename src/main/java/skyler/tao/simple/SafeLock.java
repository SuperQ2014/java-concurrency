...
public class SafeLock {
	...
	static class Friend {
		...
		public void bow(Friend bower) {
			if (impendingBow(bower)) {
				System.out.format("%s: %s is bowed to me.%n", name, bower.getName());
				bower.bowBack(this);
				lock.unlock();
				bower.lock.unlock();
			} else {
		...
		public void bowBack(Friend bower) {
			System.out.format("%s: %s is bowed back to me.%n", this.name, bower.getName());
		}
	}
	...
	public static void main(String[] args) {
		...
		new Thread(new BowLoop(lucy, lili)).start();
		new Thread(new BowLoop(lili, lucy)).start();
	}
}
